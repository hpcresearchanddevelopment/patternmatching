#ifndef HAVOQGT_LABEL_PROPAGATION_PATTERN_MATCHING_NONUNIQUE_COUNTING_EE_HPP_INCLUDED
#define HAVOQGT_LABEL_PROPAGATION_PATTERN_MATCHING_NONUNIQUE_COUNTING_EE_HPP_INCLUDED
...
public:
  ...
  std::unordered_map<VertexData, uint64_t>
    neighbor_vertex_data_count_map;
  //std::unordered_map<Vertex, uint8_t> neighbor_map; 
  // TODO: the issue is delegates are not added to the map
...
//    typedef vertex_state_generic<VertexData, DynamicBitSet, uint64_t> VertexState;
    ...
    VertexData parent_vertex_data = 0; 
                ...
                if (parent_template_vertices_bitset.test(i)) {
                  ...
                  for (auto e = pattern_graph.vertices[vertex_pattern_index]; 
                    ...
                    if (pattern_graph.edges[e] == i) {
                      ...
                      // set parent's vertex data
                      parent_vertex_data = pattern_graph.vertex_data[pattern_graph.edges[e]]; 
                      //std::cerr << "Valid parent found, vertex data " 
                      //  << parent_vertex_data << " template vertex " << pattern_graph.edges[e] 
                      //  << std::endl; // Test 
...
/*    for (size_t i = 0; i < find_vertex->second.template_neighbors.size(); i++) {
    ...
    if (find_edge == std::get<12>(alg_data)[vertex].end()) {
	  ...
	  return 0;		
        ...
        // update the neighbor vertex data count
        auto find_parent_vertex_data = 
          find_vertex->second.neighbor_vertex_data_count_map.find(parent_vertex_data); 
        if (find_parent_vertex_data == find_vertex->second.neighbor_vertex_data_count_map.end()) {    
          auto is = find_vertex->second.neighbor_vertex_data_count_map.insert({parent_vertex_data, 1});
	  if(!is.second) {
            std::cerr << "Error: failed to add an element to the map." << std::endl;
            return 0;
          }	        
        } else {
          find_parent_vertex_data->second++;
        }  
      ...
      if (std::get<8>(alg_data) == 0  && std::get<9>(alg_data)) {
        ...
        if (find_edge->second < 1) {
          ...
          if (find_parent_vertex_data == find_vertex->second.neighbor_vertex_data_count_map.end()) {
            ...
            find_parent_vertex_data->second++; 
            //std::cerr << parent_vertex_data << " " << find_parent_vertex_data->second << std::endl; // Test
          }*/
...
void verify_and_update_vertex_state(TGraph* g, AlgData& alg_data, 
  ...
  VertexIteration& vertex_iteration, uint64_t superstep, bool initstep, bool& global_not_finished, TemplateVertex& template_vertices, VertexUint8MapCollection& vertex_active_edges_map) {
  ...
  std::vector<uint64_t> vertex_remove_from_map_list; // TODO: use Vertex type, remove and use C++11
  ...
  for (auto& v : vertex_state_map) {
    ...
    for (size_t vertex_pattern_index = 0; // TODO: change the name vertex_pattern_index to template_vertex
      ...
      if (v.second.template_vertices.test(vertex_pattern_index)) {
        ...
        // verify the neighbor vertex data count          
        bool match_found = true; 
        for (auto& nd : pattern_graph.vertex_neighbor_data_count_map[vertex_pattern_index]) {
          auto find_neighbor_vertex_data = v.second.neighbor_vertex_data_count_map.find(nd.first);
          //std::cout << v.first << " " << v.second.neighbor_vertex_data_count_map.size() << std::endl;  // Test 
          if (find_neighbor_vertex_data == v.second.neighbor_vertex_data_count_map.end()) {
            // invalid
            //assert(vertex_pattern_index < 16); // Test
            //v.second.template_vertices.reset(vertex_pattern_index);
            match_found = false;
            break; 
          } else if (find_neighbor_vertex_data->second < nd.second) {
            // invalid
            //assert(vertex_pattern_index < 16); // Test  
            //v.second.template_vertices.reset(vertex_pattern_index);
            match_found = false;
            ...
            //if (superstep == 1) { // Test  
            //  std::cerr << superstep << " " << v.first << " " 
            //    << v.second.neighbor_vertex_data_count_map.size() << " "
            //    << find_neighbor_vertex_data->first << " " 
            //    << find_neighbor_vertex_data->second << " " 
            //    << nd.second << std::endl; 
            //} // Test
   
            //find_neighbor_vertex_data->second = 0; // reset for next iteration
          }  
        } // for

        if (!match_found) {
          //if (superstep == pattern_graph.diameter - 1) { // Test 
            v.second.template_vertices.reset(vertex_pattern_index);    
            continue; // move to verify the next bit in v.second.template_vertices
          //} // Test  
        } else {
          if (!v.second.neighbor_vertex_data_count_map.empty()) {
            v.second.neighbor_vertex_data_count_map.clear();
          }  
        }
...
template <typename TGraph, typename VertexMetaData, typename VertexData, typename PatternData, 
  ...
  typename VertexIteration, typename VertexStateMapGeneric, typename PatternGraph, typename BitSet, typename TemplateVertex, typename VertexUint8MapCollection>
void label_propagation_pattern_matching_bsp(TGraph* g, VertexMetaData& vertex_metadata, 
  ...
  std::ofstream& superstep_result_file, std::ofstream& active_vertices_count_result_file, TemplateVertex& template_vertices, VertexUint8MapCollection& vertex_active_edges_map) {
...
//  VertexStateMapGeneric vertex_state_map_generic;  
  ...
  auto alg_data = std::forward_as_tuple(vertex_metadata, pattern, pattern_indices, vertex_rank,
    vertex_active, vertex_iteration, vertex_state_map_generic, pattern_graph, superstep_var, initstep, g, template_vertices, vertex_active_edges_map);
  ...
  for (uint64_t superstep = 0; superstep < pattern_graph.diameter; superstep++) {
    ...
    verify_and_update_vertex_state<TGraph, decltype(alg_data), VertexStateMapGeneric,
      ...
      vertex_active, vertex_iteration, superstep, initstep, global_not_finished, template_vertices, vertex_active_edges_map);
    //MPI_Barrier(MPI_COMM_WORLD);
    //std::cout << "MPI Rank : " << mpi_rank <<  " - vertex_state_map_generic.size() : " << vertex_state_map_generic.size() << std::endl; // Test
...
#endif //HAVOQGT_LABEL_PROPAGATION_PATTERN_MATCHING_NONUNIQUE_COUNTING_EE_HPP_INCLUDED 
