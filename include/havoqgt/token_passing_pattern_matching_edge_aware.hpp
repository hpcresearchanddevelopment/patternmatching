#ifndef HAVOQGT_TOKEN_PASSING_PATTERN_MATCHING_EDGE_AWARE_HPP_INCLUDED
#define HAVOQGT_TOKEN_PASSING_PATTERN_MATCHING_EDGE_AWARE_HPP_INCLUDED
...
public:
  ...
  tppm_visitor(vertex_locator _vertex, vertex_locator _target_vertex, vertex_locator _parent, 
    size_t _itr_count, 
...
//        auto find_token_source_edge = std::get<14>(alg_data)[vertex].find(g.locator_to_label(neighbour));
//        if (find_token_source_edge == std::get<14>(alg_data)[vertex].end()) {
//          auto insert_status = std::get<14>(alg_data)[vertex].insert(g.locator_to_label(neighbour));
//          if(!insert_status.second) {
//            std::cerr << "Error: failed to add an element to the set." << std::endl;
//            return false;
//          }
          //std::cout << g.locator_to_label(vertex) << " adding " << g.locator_to_label(neighbour)
          //  << " to the vertex set" << std::endl; // Test
//        } else {
//          std::cerr << "Error: unexpected item in the set." << std::endl;
//          return false;
...
#endif //HAVOQGT_TOKEN_PASSING_PATTERN_MATCHING_EDGE_AWARE_HPP_INCLUDED
