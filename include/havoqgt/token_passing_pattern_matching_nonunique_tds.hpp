#ifndef HAVOQGT_TOKEN_PASSING_PATTERN_MATCHING_NONUNIQUE_TDS_HPP_INCLUDED
#define HAVOQGT_TOKEN_PASSING_PATTERN_MATCHING_NONUNIQUE_TDS_HPP_INCLUDED
...
bool enable_vertex_token_source_cache = false;
...
//        vertex_locator neighbor = eitr.target();
      ...
      for (auto& item : std::get<15>(alg_data)[vertex]) { // vertex_active_edges_map
        ...
        if (new_itr_count == max_itr_count) {
          if (g.locator_to_label(target_vertex) != g.locator_to_label(neighbour)) {
            continue;       
          } 
        } else if (new_itr_count < max_itr_count) {
          match_found = false;
        ...
        tppm_visitor_tds new_visitor(neighbour, vertex, target_vertex, visited_vertices, new_itr_count, max_itr_count, 
...
void token_passing_pattern_matching(TGraph* g, VertexMetaData& vertex_metadata, 
  ...
  bool pattern_selected_vertices, std::ofstream& paths_result_file) { // TODO: bool& pattern_found does not work, why?
  ...
  auto alg_data = std::forward_as_tuple(vertex_metadata, pattern, pattern_indices, vertex_rank, 
    ...
    pattern_selected_vertices, paths_result_file);
...
#endif //HAVOQGT_TOKEN_PASSING_PATTERN_MATCHING_NONUNIQUE_TDS_HPP_INCLUDED
