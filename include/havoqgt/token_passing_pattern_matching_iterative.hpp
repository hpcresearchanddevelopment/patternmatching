#ifndef HAVOQGT_TOKEN_PASSING_PATTERN_MATCHING_ITERATIVE_HPP_INCLUDED
#define HAVOQGT_TOKEN_PASSING_PATTERN_MATCHING_ITERATIVE_HPP_INCLUDED
...
*/
      // if vertex is in token_source_map, create a token
      auto find_token_source = std::get<6>(alg_data).find(g.locator_to_label(vertex)); // token_source_map      
      if (find_token_source == std::get<6>(alg_data).end()) {
        return false;  
      } 
...
//          }  
          ...
          return true; // Importnat : must return true to handel delegates
...
void token_passing_pattern_matching(TGraph* g, VertexMetaData& vertex_metadata, 
  ...
  VertexSetCollection& vertex_token_source_set, VertexActive& vertex_active) { // TODO: bool& pattern_found does not work, why?
  ...
  for (auto& v : vertex_state_map) { // TODO: iterate over the local and delegates
    auto v_locator = g->label_to_locator(v.first);
    if (v.second.vertex_pattern_index == pattern_indices[0] && vertex_metadata[v_locator] == pattern[0]) {
      auto find_token_source = token_source_set.find(v.first);
      if (find_token_source == token_source_set.end()) {
        auto insert_status = token_source_set.insert(v.first);
        if(!insert_status.second) {
          std::cerr << "Error: failed to add an element to the set." << std::endl;
          return false;
        }
  ...
  //std::cout << "token_source_set size " << token_source_set.size() << std::endl; // Test    
//  uint64_t global_vertex_state_map_size = havoqgt::mpi::mpi_all_reduce(vertex_state_map.size(), std::greater<size_t>(), MPI_COMM_WORLD);
//  MPI_Barrier(MPI_COMM_WORLD); 
  //std::cout << "global_vertex_state_map_size " << global_vertex_state_map_size << std::endl; // Test 
  ...
  if (mpi_rank == 0) {
    std::cout << "Token Passing | Global Token Source Count : " 
      << global_token_source_set_size << std::endl; // Test
  }

  // TODO: default should be starting tokens from
  uint64_t max_batch_size = 1;
  uint64_t batch_size = 0;
  uint64_t batch_count = 0;
  uint64_t max_ranks_per_itr = (mpi_size / 24) * 2;
  ...
  // TODO: this range is considered only once, how about multiple times 

    for (auto v = token_source_set.cbegin(); v != token_source_set.cend();) { // populate the token_source_map
      //if (batch_size == 0) {
      //  token_source_map.clear();
      //}
      ...
      if (find_token_source == token_source_map.end()) {
        ...
        v = token_source_set.erase(v); // remove entry         
        //batch_size++;
      } else {
        std::cerr << "Error: unexpected item in the set." << std::endl;
        //return false;  
  ...
  // Test
  uint64_t global_batch_token_source_map_size = havoqgt::mpi::mpi_all_reduce(token_source_map.size(), std::plus<size_t>(), MPI_COMM_WORLD);
  MPI_Barrier(MPI_COMM_WORLD);
  ...
  for (auto& s : token_source_map) {
    ...
    if (!vertex_active[v_locator]) {
      // TODO: check if vertex is in the map 
...
#endif //HAVOQGT_TOKEN_PASSING_PATTERN_MATCHING_ITERATIVE_HPP_INCLUDED
