#ifndef HAVOQGT_TOKEN_PASSING_PATTERN_MATCHING_NONUNIQUE_NEM_2_HPP_INCLUDED
#define HAVOQGT_TOKEN_PASSING_PATTERN_MATCHING_NONUNIQUE_NEM_2_HPP_INCLUDED
...
public:
  ...
  bool pre_visit(AlgData& alg_data) const {
    ...
    // std::get<17>(alg_data); // pattern_verify_join_vertex 
    size_t pattern_join_vertex = std::get<18>(alg_data); //1; // TODO: // pattern_join_vertex 
...
//+           }  
           ...
           if (do_pass_token && (max_itr_count > itr_count)) {
             ...
             if (!(vertex.is_delegate() && (g->master(vertex) != mpi_rank))) {  
               if(std::get<17>(alg_data) && (pattern_join_vertex == vertex_pattern_index)) {
               ...
               } /*else if (std::get<17>(alg_data) && (pattern_join_vertex == vertex_pattern_index)) {
                 // find vertex in the vertex_state_map and verify if it is marked
                 auto find_vertex = std::get<5>(alg_data).find(g->locator_to_label(vertex));
                 if (find_vertex == std::get<5>(alg_data).end()) {
                   std::cerr << "Error: did not find the expected item in the map." << std::endl;
                   return false;
                   ...
                   if (!find_vertex->second.is_active) {
                     return false; // do not forward token
                   }     
                 } 
               }*/            
             } 
        ...
        if (match_found && !expect_target_vertex) {
          if (g.locator_to_label(vertex) == g.locator_to_label(target_vertex)) {
            ...
            if (std::get<16>(alg_data)) { // pattern_selected_vertices
              ...
              tppm_visitor new_visitor(target_vertex, target_vertex, itr_count, max_itr_count,
...
//        vertex_locator neighbor = eitr.target();
      ...
      for (auto& item : std::get<15>(alg_data)[vertex]) { // vertex_active_edges_map
        ...
        tppm_visitor new_visitor(neighbour, target_vertex, new_itr_count, max_itr_count, 
...
void token_passing_pattern_matching(TGraph* g, VertexMetaData& vertex_metadata, 
  ...
  bool pattern_selected_vertices, bool pattern_verify_join_vertex, size_t pattern_join_vertex) { // TODO: bool& pattern_found does not work, why?
  ...
  auto alg_data = std::forward_as_tuple(vertex_metadata, pattern, pattern_indices, vertex_rank, 
    ...
    pattern_selected_vertices, pattern_verify_join_vertex, pattern_join_vertex);
...
#endif //HAVOQGT_TOKEN_PASSING_PATTERN_MATCHING_NONUNIQUE_NEM_2_HPP_INCLUDED
