...
//--  VertexSetCollection token_source_edge_set(*graph); // per vertex set // edge aware
  ...
  // TODO: can you draw the pattern graphically using a 2D graphics library?
  // print pattern  
  if(mpi_rank == 0) {
  ...
  vertex_active_edges_map.clear(); // important // TODO: move active edges map in the vertex state 
  ...
  uint64_t active_vertices_count = 0; // per MPI rank
...
//--      << pattern_seleted_edges_tp << std::endl; // Test 		
  ...
  if (!pattern_selected_vertices_tp) {
    ...
    // delete invalid vertices from the token_source_set
    // reset the valid vertices in the token_source_set 
    ...
    for (vitr_type vitr = graph->vertices_begin(); 
      ...
      vloc_type vertex = *vitr;
      if (vertex_active[vertex] && 
        (vertex_metadata[vertex] ==  pattern_tp[0])) {
  ...
  for (auto& s : token_source_map) {
    if (!s.second) {
      ...
      if (v_template_vertices.test(pattern_indices_tp[0])) {
        assert(pattern_indices_tp[0] < 16); // Test
