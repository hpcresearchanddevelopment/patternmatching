...
#include <havoqgt/pattern_graph_util.hpp>
...
#include <havoqgt/token_passing_pattern_matching_nonunique_nem_2.hpp> // TP_ASYNC // nonunique path checking, edge monocyclic 
...
//--  VertexSetCollection token_source_edge_set(*graph); // per vertex set // edge aware
  ...
  typedef pattern_graph_csr<Vertex, Edge, VertexData, EdgeData> PatternGraph; // TODO: fix graph class name conflict
  PatternGraph pattern_graph(mpi_rank, pattern_input_filename + "_edge",
  ...
  // TODO: Should 'Vertex' type be 16-bit to match bit vector length? 
  pattern_util<VertexData, Vertex> ptrn_util_two(pattern_input_filename + "_nem", 
    pattern_input_filename + "_join_vertex", true);
  ...
  if(mpi_rank == 0) {
    std::cout << "Fuzzy Pattern Matching | Initializing Containers ... " << std::endl;
  }
  ...
  if(mpi_rank == 0) {  
    std::cout << "Fuzzy Pattern Matching | Done Initializing Containers." << std::endl;
  }
  ...
  if(mpi_rank == 0) {
    std::cout << "Fuzzy Pattern Matching | Running Core Algorithms ... " << std::endl;
  } 
...
//   (graph, vertex_metadata, pattern, pattern_indices, vertex_rank, vertex_active,
//   vertex_iteration, vertex_state_map, pattern_graph, global_initstep, global_not_finished,
//   global_itr_count, superstep_result_file, active_vertices_count_result_file, template_vertices, vertex_active_edges_map);
...
//--  auto pattern_seleted_edges_tp = std::get<5>(ptrn_util_two.input_patterns[pl]); // boolean 
  ...
  // token passing options // TODO:
  bool pattern_verify_join_vertex_tp = true;   
  size_t pattern_join_vertex_tp = ptrn_util_two.join_vertices[pl]; //1; // assuming uinique vertex data for the join vertex 
...
//--      << pattern_seleted_edges_tp << std::endl; // Test 		
    ...
    std::cout << "Token Passing [" << pl << "] | Join Vertex : " 
      << ptrn_util_two.join_vertices[pl] << std::endl;
    //std::cout << "Token Passing [" << pl << "] | Join Vertices : "; 	
    //for (auto v : ptrn_util_two.join_vertices) {     	
    //  std::cout << v << " ";
    //}
    //std::cout << std::endl;
  }
...
//    pattern_found[pl], edge_active, /**edge_data_ptr,*/ vertex_token_source_set, token_source_edge_set, vertex_active); 
  token_passing_pattern_matching<graph_type, VertexMetaData, decltype(pattern), decltype(pattern_indices), uint8_t, PatternGraph,
    ...
    pattern_verify_join_vertex_tp, pattern_join_vertex_tp);
#endif
...
//  }
  ...
  // TODO: this is a temporary patch, eliminate the invalid join vertices
  // "remove from vertex_state_map" could be done inside the following loop   
  for (auto itr = vertex_state_map.begin(); itr != vertex_state_map.end();) {
    // assuming uinique vertex data for the join vertex
    auto vertex = graph->label_to_locator(itr->first); 
    if(!vertex_active[vertex] || !(vertex_metadata[vertex] == pattern_tp[pattern_join_vertex_tp])) {
      ++itr;
      continue;
    }

    BitSet vertex_template_vertices(template_vertices[vertex]);
    if (vertex_template_vertices.none() || !vertex_template_vertices.test(pattern_join_vertex_tp)) {
      ++itr;
      continue;
    }        
  
    if(!itr->second.is_active) {
      itr = vertex_state_map.erase(itr); // C++11
    } else {
      itr->second.is_active = 0; // reset for the next iteration 
      ++itr; 
    }   
  }      
