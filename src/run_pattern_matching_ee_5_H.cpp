...
//--  VertexSetCollection token_source_edge_set(*graph); // per vertex set // edge aware
  ...
  //pattern_util<VertexData, Vertex> ptrn_util_two(pattern_input_filename + "_nem", pattern_input_filename + "_tds", true, true);
...
//--  auto pattern_seleted_edges_tp = std::get<5>(ptrn_util_two.input_patterns[pl]); // boolean 
  ...
  //auto pattern_enumeration_tp = ptrn_util_two.enumeration_patterns[pl]; // tds_1.hpp 
  ...
  //if (pl >= 1000) {
  // /p/lscratchf/havoqgtu/reza2_tmp/IMDB/graph/patterns/
  //if (pl >= 0) {
  // /p/lscratchf/havoqgtu/reza2_tmp/IMDB/graph/patterns_B_2/
  if (pl >= 16 && pl <=27) {
    do_tds_tp = true;
    if(mpi_rank == 0) {
      std::cout << "Token Passing [" << pl << "] | Enumeration " << std::endl;
    }
  }  
  // Test
...
//--      << pattern_seleted_edges_tp << std::endl; // Test 		
    ...
    //pattern_util<Vertex>::output_pattern(pattern_enumeration_tp); // tds_1.hpp  
  }
...
//    pattern_found[pl], edge_active, /**edge_data_ptr,*/ vertex_token_source_set, token_source_edge_set, vertex_active);
  if (do_tds_tp) {
    //token_passing_pattern_matching_nonunique_tds.hpp 
    token_passing_pattern_matching<graph_type, Vertex, VertexMetaData, 
      decltype(pattern), decltype(pattern_indices), uint8_t, PatternGraph,
      VertexStateMap, TokenSourceMap, edge_data_t,
      VertexSetCollection, VertexActive, TemplateVertex, VertexUint8MapCollection, 
      BitSet>
      (graph, vertex_metadata, pattern_tp, pattern_indices_tp, vertex_rank, 
      pattern_graph, vertex_state_map, token_source_map, pattern_cycle_length_tp, 
      pattern_valid_cycle_tp, pattern_found[pl], *edge_data_ptr, 
      vertex_token_source_set, vertex_active, template_vertices, 
      vertex_active_edges_map, pattern_selected_vertices_tp, paths_result_file);
    ...
    /*token_passing_pattern_matching<graph_type, Vertex, VertexMetaData, 
      ...
      uint8_t, PatternGraph, VertexStateMap, TokenSourceMap, edge_data_t,
