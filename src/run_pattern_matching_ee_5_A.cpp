...
#include <havoqgt/token_passing_pattern_matching_nonunique_tds.hpp> // TP_ASYNC // nonunique path checking, edge monocyclic, tds
...
//--  VertexSetCollection token_source_edge_set(*graph); // per vertex set // edge aware
  ...
  if (use_degree_as_vertex_data) {
    ...
    vertex_data_db<graph_type, VertexMetaData, Vertex, VertexData>
      ...
      // TODO: each rank reads at most 10K lines at a time from a file
  }
  ...
  // mark inactive edges // TODO: remove
  //update_edge_state();
...
//--  auto pattern_seleted_edges_tp = std::get<5>(ptrn_util_two.input_patterns[pl]); // boolean 
  ...
  // Test // Reddit Pattern 25
  if (pl == 100) {
    pattern_mark_join_vertex_tp = true;
    pattern_ignore_join_vertex_tp = false; 
    pattern_join_vertex_tp = 2; 
  } else if (pl == 101) {
    pattern_mark_join_vertex_tp = false;
    pattern_ignore_join_vertex_tp = true;
    pattern_join_vertex_tp = 5;
  } else if (pl == 102) {   
    pattern_selected_edges_tp = true;    
  } else if (pl >= 4) {
    do_tds_tp = true;	
    if(mpi_rank == 0) {
      std::cout << "Token Passing [" << pl << "] | TDS" << std::endl; 
    }
  }// Test

/*  // Test // Reddit Pattern 26
  if (pl == 100) {
    pattern_mark_join_vertex_tp = true;
    pattern_ignore_join_vertex_tp = false; 
    pattern_join_vertex_tp = 2; 
    ...
    pattern_ignore_join_vertex_tp = true;
    pattern_join_vertex_tp = 5;
  } else if (pl == 21) { // Important : verify  
    pattern_selected_edges_tp = true;    
  } else if (pl >= 13 && pl <= 19) {
    do_tds_tp = true;	
    if(mpi_rank == 0) {
...
//    pattern_found[pl], edge_active, /**edge_data_ptr,*/ vertex_token_source_set, token_source_edge_set, vertex_active);
  if (do_tds_tp) {
      token_passing_pattern_matching<graph_type, Vertex, VertexMetaData, decltype(pattern), decltype(pattern_indices), uint8_t, PatternGraph,
    ...
    template_vertices, vertex_active_edges_map, pattern_selected_vertices_tp, paths_result_file); 
  } else {     
  token_passing_pattern_matching<graph_type, VertexMetaData, decltype(pattern), decltype(pattern_indices), uint8_t, PatternGraph,
    ...
    pattern_ignore_join_vertex_tp, pattern_join_vertex_tp);
  }
#endif
...
//  }
  ...
  if (pattern_selected_vertices_tp) { // the last vertex is added to the token_source_map
  ...
  for (auto& s : token_source_map) {
	...
	token_source_deleted = true;
      ...
      // reset edge active state
      if (pattern_interleave_label_propagation_tp) { // TODO: pattern_interleave_label_propagation_tp is not the most appropriate flag here
        auto v_locator = graph->label_to_locator(s.first);
...
//--  }
  ...
  // pattren found ? // TODO: write output to file
  // TODO: probably do not need mpi_all_reduce_inplace here 
  havoqgt::mpi::mpi_all_reduce_inplace(pattern_found, std::greater<uint8_t>(), MPI_COMM_WORLD);
  MPI_Barrier(MPI_COMM_WORLD); // TODO: do we need this here?   
  if(mpi_rank == 0) {   
    for (size_t pl = 0; pl < ptrn_util_two.input_patterns.size(); pl++) {
